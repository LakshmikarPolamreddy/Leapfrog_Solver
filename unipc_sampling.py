...
output_folder = "/scratch/laks/leapfrog_solver/samples/imagenet/unipc_sampling"
...
for class_label in range(start_class, end_class + 1):
    ...
    with torch.no_grad():
        with model.ema_scope():
            ...
            print(f"Rendering {n_samples_per_class} examples of class '{class_label}' in {unipc_steps} steps and using s={scale:.2f}.")
            xc = torch.tensor(n_samples_per_class*[class_label])
            c = model.get_learned_conditioning({model.cond_stage_key: xc.to(model.device)})
            samples_unipc, _ = sampler.sample(S=unipc_steps,
                                                 conditioning=c,
                                                 ...
                                                 eta=unipc_eta)
            x_samples_unipc = model.decode_first_stage(samples_unipc)
            x_samples_unipc = torch.clamp((x_samples_unipc+1.0)/2.0, 
                                              min=0.0, max=1.0)
            all_samples.append(x_samples_unipc)
